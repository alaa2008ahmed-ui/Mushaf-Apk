...
import { encryptBackupData, decryptBackupData } from '../utils';
import { Database, Download, Upload, ShieldCheck, AlertTriangle, CheckCircle, FileText, Building2, Save, Palette, Check, Calculator, Lock } from 'lucide-react';
...
interface Props {
  employees: Employee[];
  onRestoreBackup: (employees: Employee[]) => void;
}

export default function SettingsView({ employees, onRestoreBackup }: Props) {
  ...
  const handleCreateBackup = () => {
    try {
      ...
      const encryptedContent = encryptBackupData(employees);
      const blob = new Blob([encryptedContent], { type: 'application/octet-stream' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      const dateStr = new Date().toISOString().slice(0, 10);
      a.href = url;
      a.download = `adba_water_backup_${dateStr}.bak`;
      ...
      setStatusMessage({
        type: 'success',
        text: 'تم إنشاء وتحميل ملف النسخة الاحتياطية المشفرة (.bak) بنجاح.'
      });
    } catch (err: any) {
      setStatusMessage({
        type: 'error',
        text: err.message || 'حدث خطأ أثناء إنشاء النسخة الاحتياطية.'
      });
    }
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith('.bak')) {
      setStatusMessage({
        type: 'error',
        text: 'يرجى اختيار ملف نسخة احتياطية صالح بامتداد (.bak).'
      });
    ...
    reader.onload = (event) => {
      try {
        ...
        const decryptedData = decryptBackupData(content);
        if (!Array.isArray(decryptedData)) {
          throw new Error('تنسيق البيانات في ملف النسخة الاحتياطية غير صحيح.');
        }

        onRestoreBackup(decryptedData);
        if (fileInputRef.current) fileInputRef.current.value = '';
        setStatusMessage({
          type: 'success',
          text: 'تم التحقق من النسخة الاحتياطية. جاري استرداد البيانات (بعد تأكيد صلاحية كلمة السر)...'
        });
      } catch (err: any) {
        setStatusMessage({
          type: 'error',
          text: err.message || 'فشل استرداد النسخة الاحتياطية. تأكد من سلامة الملف المشفر.'
        });
        if (fileInputRef.current) fileInputRef.current.value = '';
      }
    };
  ...
  return (
    <div className="p-4 sm:p-6 w-full flex-grow flex flex-col justify-start text-right font-sans" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 hover:border-indigo-200 transition-colors flex flex-col justify-between md:col-span-1">
          <div className="flex items-center gap-2 mb-2 pb-1 border-b border-slate-100">
            <div className="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600 shrink-0">
              <Download className="w-4 h-4" />
            </div>
            <h2 className="text-xs font-bold text-slate-800">إنشاء نسخة احتياطية (Backup)</h2>
          </div>

          <button
            onClick={handleCreateBackup}
            className="w-full py-1.5 px-3 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-bold rounded-lg shadow-sm transition-all flex items-center justify-center gap-1.5 text-xs mt-2"
          >
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 hover:border-indigo-200 transition-colors flex flex-col justify-between md:col-span-1">
          <div className="flex items-center gap-2 mb-2 pb-1 border-b border-slate-100">
            ...
            <h2 className="text-xs font-bold text-slate-800">استرداد البيانات (Restore)</h2>
          </div>
          ...
          <div>
            ...
            <button
              ...
              className="w-full py-1.5 px-3 bg-slate-800 hover:bg-slate-900 active:bg-black text-white font-bold rounded-lg shadow-sm transition-all flex items-center justify-center gap-1.5 text-xs mt-2"
            >
