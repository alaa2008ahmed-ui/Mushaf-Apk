...
export default function VacationAllowanceView({ employees, onArchive, archivedData }: Props) {
  ...
  if (originalEmp) {
    ...
    if (isProportionalActive) {
      ...
      otherAllowancesValue = Math.min((emp.fixedAllowances || 0), Math.round((((emp.fixedAllowances || 0) / 30) * effectiveDays) * 100) / 100);
              ...
              <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg animate-in fade-in duration-200">
                <div className="flex items-center gap-1.5 text-xs font-bold text-amber-900">
                  ...
                  <input 
                    ...
                    max="30" 
                    value={workDaysCount} 
                    onChange={(e) => setWorkDaysCount(Math.min(30, Math.max(0, Number(e.target.value))))} 
                    className="w-14 bg-white border border-amber-400 rounded px-1.5 py-0.5 text-center font-black text-amber-950 shadow-inner focus:outline-none focus:ring-2 focus:ring-amber-500" 
                  />
                  <span>يوم</span>
                </div>
